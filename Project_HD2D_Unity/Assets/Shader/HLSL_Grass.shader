Shader "Custom/HLSL_Grass"
{
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			GeomData TransformGeomToClip(float3 posOS, float3 offsetOS, float3x3 m, float2 uv)
			{
				GeomData o;
			...
			{
				float2 grassUV = TRANSFORM_TEX(input[0].uv, _GrassMap);
				[unroll]
				for (int v = 0; v < 3; v++)
				{
					float2 grassUV = TRANSFORM_TEX(input[v].uv, _GrassMap);
					float grassVisibility = tex2Dlod(_GrassMap, float4(grassUV, 0, 0)).r;

					if (grassVisibility < _GrassThreshold)
						continue;

					float3 pos = input[v].vertex.xyz;
					float3 normal = input[v].normal;
					float4 tangent = input[v].tangent;
					...
					float2 windSample2 = (tex2Dlod(_WindMap, float4(windUV, 0, 0)).xy * 2 - 1) * length(_WindVelocity);

					float3 windAxis = normalize(float3(windSample2.x, windSample2.y, 0));
					float windAmt = length(windSample2);
					float3x3 windMatrix = angleAxis3x3(UNITY_PI * windAmt, windAxis);

					float3x3 baseM = mul(tangentToLocal, randRotMatrix);
					float3x3 tipM  = mul(mul(mul(tangentToLocal, windMatrix), randBendMatrix), randRotMatrix);
					...
					for (int s = 0; s < BLADE_SEGMENTS; ++s)
					{
						...
						triStream.Append(TransformGeomToClip(pos, float3( offset.x, offset.y, offset.z), m, float2(0, t)));
						triStream.Append(TransformGeomToClip(pos, float3(-offset.x, offset.y, offset.z), m, float2(1, t)));
					}

					triStream.Append(TransformGeomToClip(pos, float3(0, forward, height), tipM, float2(0.5, 1)));
		...
		Pass
		{
			...
			float4 frag(GeomData i) : SV_Target
			{
				...
				float4 albedo = tex2D(_BladeTexture, i.uv);
